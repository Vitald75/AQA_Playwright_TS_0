...
test('Site2 using getBy. E2E - login, add to cart and purchase.', async ({page})=>
{
    const productName = 'ZARA COAT 3';
    const userName = page.getByPlaceholder('email@example.com'); // locator('input[id="userEmail"]');
    const password = page.getByPlaceholder('enter your passsword') //locator('input[id="userPassword"]');
    const products = page.locator('[class="card-body"]');
    const cartButton = page.getByRole('listitem').getByRole('button', {name: 'Cart'}); //locator('button[routerlink="/dashboard/cart"]');

    const checkoutButton = page.getByRole('button', {name : 'Checkout'}); //locator('div.subtotal button[type="button"]');
    
    const creditCard = page.locator('i.icon-credit-card');
    const selectCountryInput = page.getByPlaceholder('Select Country'); //locator('input[placeholder="Select Country"]');
    ...
    await page.getByRole('button', {'name' : 'login'}).click(); //locator('input[id="login"]').click();
    ...
    await expect(page.getByText('Your Orders')).toBeVisible(); //await expect(page.locator('h1.ng-star-inserted').first()).toHaveText('Your Orders');
